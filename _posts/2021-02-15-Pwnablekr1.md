...
So we can approach this by dividing 0x21DD09EC by 5 but it's not dividable. After converting it to decimal and doing some basic math we come up with this: 568134124 = 4 x 113626824 + 113626828.
...
The comarison happens at ebp+0x8 (cmp    DWORD PTR [ebp+0x8],0xcafebabe). That's what we want to overwrite with 0xcafebabe.

Let's run and give it 100 bytes and see if we hit ebp+8.
